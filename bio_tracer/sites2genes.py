...
@DryPipe.python_call()
def sites2genes(mappable_features, mappable_intersect_bed, genes_insertions_tsv):
    # Read files
    gene_df = pd.read_csv(
        mappable_features,
        dtype={
    ...
    intersect_df = pd.read_csv(
        ...
        usecols=range(10),
        names=[
            "chr_name_gene", "start_gene", "end_gene", "name_gene", "score_gene", "strand_gene",
            "chr_name_insertion", "start_insertion", "end_insertion", "score_insertion"
        ],
    )
    ...
    gene_df[[
        "chr_name", "locus_tag", "gene_name", "ncrna", "start", "end", "strand",
        "read_count", "ins_index", "gene_length", "ins_count", "fcn"]
    ].to_csv(
